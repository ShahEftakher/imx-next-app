...
const Home: NextPage = () => {
  ...
  const transferAsset = async () => {
    const ethSigner = await getSigner();
    const starkPK = generateStarkPrivateKey();
    console.log(starkPK);
    const starkSigner = createStarkSigner(starkPK);
    console.log(starkSigner);
    //how to configure a starksigner
    //existing pk are not available to us
    const link = new Link(NEXT_APP_SANDBOX_LINK_URL);
    const user = await link.setup({});
    // const sk = StarKey
    try {
      const result = await client.transfer(
        { ethSigner, starkSigner },
        {
          type: 'ERC721',
          receiver: '0x8aE41d03aac4f8D0f8eda73273B0dCFe40306f34',
